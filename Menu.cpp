...
void Menu::Init()
{
	...
	inputmanager.InputMapping("SelectButton", SDLK_RETURN);
	inputmanager.InputMapping("NextButton", SDLK_DOWN);
	inputmanager.InputMapping("PrevButton", SDLK_UP);
	bg_menu.BuildBackground(GetScreenWidth(),GetScreenHeight(),"bg.png");
	pisang.BuildSprite(GetScreenWidth(), GetScreenHeight(), "banana.png", 0.1f);
}
...
{
	if (inputmanager.IsKeyDown("SelectButton")) {
	...
	if (inputmanager.IsKeyUp("NextButton")) {
	...
	if (inputmanager.IsKeyUp("PrevButton")) {
		if (activeButtonIndex > 0) {
			activeButtonIndex = activeButtonIndex - 1;
			SDL_Delay(150);
		}
...
void Menu::Render()
{
	...
	bg_menu.DrawBackground();
	RenderText("   CATCH THE FRUIT", GetScreenWidth() / 4, 50, 1.0f, vec3(255.0f / 255.0f, 225.0f / 255.0f, 0.0f / 255.0f));
...
void Menu::InitText() {
	...
	for (GLubyte c = 0; c < 128; c++)
	{
		...
		Characters.insert(pair<GLchar, Character2>(c, character));
	}
...
void Menu::RenderText(string text, GLfloat x, GLfloat y, GLfloat scale, vec3 color)
{
...
void Menu::InitButton() {
	string buttons[NUM_BUTTON] = { "Play_Button_Stand_By.png", "Exit_Button_Stand_By.png" };
	string hover_buttons[NUM_BUTTON] = { "Play_Button_Selected.png", "Exit_Button_Selected.png" };
